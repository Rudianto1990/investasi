...
class Laporan_investasi extends CI_Controller {
	...
	public function index()
	{
        ...
        $data['hasil'] = $this->crud->getFilterInvestasi($from,$to);
  		

		$main->display("laporan/investasi/index",$data);
	...
	function export_excel(){
      ...
      $object->getActiveSheet()->setCellValue('A1', 'LAPORAN INVESTASI - IPC Pelabuhan II Kantor Cabang Priok');
		  $object->getActiveSheet()->mergeCells('A1:K1');

		  $object->getActiveSheet()->setCellValue('A2', 'Alamat Kantor : Jl. Pasoso');
      ...
      foreach($table_columns as $field){
		...
		// set data
      $result_data = $this->crud->getFilterInvestasi($from,$to);
      ...
      foreach($result_data as $row){
      		...
      		// untuk indikator
      		// $cek_adendum = $this->crud->getAdendumInvestasiBy($row->investasi_id);
                      
        // 	if($cek_adendum['created_date'] == null)
          if($row->investasi_selesai_pekerjaan != null)
        	{
                                $tanggal1 = new DateTime(date("Y-m-d"));
                                $tanggal2 = new DateTime(date("Y-m-d",strtotime($row->investasi_selesai_pekerjaan)));
        ...
        $object->getActiveSheet()->setCellValueByColumnAndRow(1, $excel_row, $row->investasi_pr_number);

        $object->getActiveSheet()->setCellValueByColumnAndRow(2, $excel_row, $row->investasi_po_number);

        $object->getActiveSheet()->setCellValueByColumnAndRow(3, $excel_row, $row->bidang_nama);

        $object->getActiveSheet()->setCellValueByColumnAndRow(4, $excel_row, $row->investasi_uraian_pekerjaan);

        $object->getActiveSheet()->setCellValueByColumnAndRow(5, $excel_row, $row->investasi_no_kontrak);

        $object->getActiveSheet()->setCellValueByColumnAndRow(6, $excel_row, number_format($row->investasi_nilai_pekerjaan,0,',','.'));

        $object->getActiveSheet()->setCellValueByColumnAndRow(7, $excel_row, date("d/m/Y", strtotime($row->investasi_mulai_pelaksanaan)));

        $object->getActiveSheet()->setCellValueByColumnAndRow(8, $excel_row, date("d/m/Y", strtotime($row->investasi_selesai_pekerjaan)));

        $object->getActiveSheet()->setCellValueByColumnAndRow(9, $excel_row, date("d/m/Y", strtotime($row->investasi_tanggal_jaminan_pelaksanaan)));
      ...
      $judul = "Laporan-investasi_from-".$judulfrom."_to-".$judulto.".xls";
    ...
    function cetak_pdf()
    {
        ...
        $this->load->view('laporan/investasi/cetak_investasi', $data);
        ...
        // $this->dompdf->stream("LAPORAN_INVESTASI-".$tgl.".pdf",array('Attachment'=>0));
        $this->dompdf->stream("LAPORAN-INVESTASI_from-".$judulfrom.'_to-'.$judulto.".pdf",array('Attachment'=>0));
