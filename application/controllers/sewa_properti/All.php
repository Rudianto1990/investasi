...
class All extends CI_Controller {
	...
	public function index()
	{
		...
		$data['title'] = 'Penyewaan Properti - All';
		$data['subtitle'] = '';
        $data['crumb'] = [
            'Sewa Properti' => '',
            'All' => '',
		...
		$main->display("sewa_properti/all/index",$data);
	
	}


	function get_data_sp_all()
    {
    	$list = $this->dtable->get_datatables_sp_all();
		$data = array();
		$no = $_POST['start'];
		...
		foreach ($list as $field) {
			...
			if($field->tgl_batas_pembayaran != null)
			{
				$tanggal1 = date_create(date("Y-m-d"));
                $tanggal2 = date_create(date("Y-m-d",strtotime($field->tgl_batas_pembayaran)));
                $selisih_hari = date_diff($tanggal1,$tanggal2);

                $infoTagihan = "<span class='label label-default' style='background:#777;color:#fafafa;'>".$selisih_hari->format("%r%a")." hari lagi</span>";
			}
			//echo $selisih_hari;
			else
			{
		...
		$output = array(
			"draw" => $_POST['draw'],
			"recordsTotal" => $this->dtable->count_all_sp_all(),
			"recordsFiltered" => $this->dtable->count_filtered_sp_all(),
			"data" => $data,
		);
